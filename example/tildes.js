...
export default function createPlot (context, dimensions) {
  ...
  for (let θ = 0; θ < max; θ += dθ) {
    ...
    x = xSpiral + pow(cos(xSpiral / sizeX /θ), 2) * (cos(ySpiral / sizeX/θ)) * (sin(ySpiral / sizeX)) * sizeY/1.3;
    y = ySpiral + sin(xSpiral / sizeY) * cos(xSpiral / sizeY /θ) * (sin(ySpiral / sizeY - PI / 2)) * (cos(ySpiral / sizeX)) * sizeY / 1.3;
